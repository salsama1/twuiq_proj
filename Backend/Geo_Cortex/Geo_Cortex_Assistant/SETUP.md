...
### Test Workflow Agent (recommended for judges)

Use the `/agent/workflow` endpoint (returns a plan + tool trace):

```json
{
  "query": "Run QC summary and explain what it means",
  "max_steps": 4,
  "use_llm": true
}
```

For a fast/offline demo (no Ollama needed):

```json
{
  "query": "Run QC summary",
  "max_steps": 3,
  "use_llm": false
}
```
...
Optional:
- `LLM_TIMEOUT_SEC=20` (hard timeout for LLM calls)
- `LLM_DISABLED=true` (disable LLM calls entirely; endpoints still run)
